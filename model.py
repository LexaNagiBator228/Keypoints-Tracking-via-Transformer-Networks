...
class Final_net_small(nn.Module):
    def __init__(self,depth, dim, head, d_q, d_v,kp_in=5,  dropout_rate=0.2 ):
        super(Final_net_small, self).__init__()
        self.prep_im = MLP([dim,d_q,d_q])
        self.prep_p = MLP([dim,d_q,d_q])
        self.pos_enc = MLP([kp_in, 32,64,128,d_q//2, d_q])
        ...
        self.poins_plus_im = Trans_for_two(depth, d_q, head, d_q,d_q )
    ...
    def forward(self,im1,im2,poitns, im_points, points_pos, ocl = False):
        #### Pos encoding ####
        im1 = self.prep_im(im1) + self.pos_enc(im_points)
        im2 = self.prep_im(im2) + self.pos_enc(im_points)
        poitns = self.prep_p(poitns) + self.pos_enc(points_pos)

        b = poitns.shape[0]
        cls_occl = self.occl.expand(b,-1,-1)
        
        im2 = torch.cat((im2, cls_occl), dim =1)
        #### Preproc points and img ####


        poitns = self.self_att_points(poitns)
...
class Linear_net_small(nn.Module):
    def __init__(self,depth, dim, head, d_q, d_v,kp_in=5,  dropout_rate=0.2 ):
        ...
        self.poins_plus_im =LocalFeatureTransformer(def_cofig) #Trans_for_two(depth, d_q, head, d_q,d_q )
...
class fine_mod(nn.Module):
    def __init__(self,depth, dim, head):
        super(fine_mod, self).__init__()
        ########################## Trans Part ##################
        ...
        self.f_pos = nn.Parameter(torch.zeros(1, 1, dim))
        self.point_pos = nn.Parameter(torch.zeros(1, 9, dim))
        self.f_encode = MLP([dim,dim,dim], do_bias=False )
        self.res =  MLP([9,9,9,9,2], do_bias=True)
        self.d_q = dim
        self.tanh = nn.Tanh()



    def forward(self,im_points, f_origin, f_after):
        #### Pos encoding ####


        b = im_points.shape[0]
        pos_token = self.point_pos.expand(b,-1,-1)
        im_points =im_points + pos_token


        f_after = torch.nn.functional.normalize(f_after, p=2, dim=1)
        f_after = self.f_encode(f_after)
        f = f_origin + f_after
        f= f.view(f.shape[0], 1, f.shape[1])


        p, im =  self.poins_plus_im(f, im_points)

        im = im.transpose(1,2)
        scores = torch.matmul(p,im)/p.shape[2]**.5
        scores = scores.view(scores.shape[0], scores.shape[2])
        
        res = self.tanh(self.res(scores))
